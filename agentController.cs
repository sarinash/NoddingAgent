...
public class MicrophoneInput : MonoBehaviour
{
    ...
    public float fix= 0.0001f;
    List<float> audiolist = new List<float>(); //list to store audio per frame
    #region SingleTon
    //int Play=0;

    public static MicrophoneInput Inctance { set; get; }
    ...
    //changing the intervals here 6f= duration-1 and 7f= duration

    private string _device;
    void Start(){

        InvokeRepeating ("Emptylist", 6f, 7f); 
        ...
        if(average>=0.0001)
            {
                Debug.Log("ok");
                //Play=1;
                animator.SetBool("emo1",true); 
                
                
                Invoke("finishEmo1", 5f);
            }
    ...
    public void finishEmo1()=>animator.SetBool("emo1", false);
    ...
    void Update()
    {
        ...
        audiolist.Add(MicLoudness);
        average = audiolist.Average() ;
